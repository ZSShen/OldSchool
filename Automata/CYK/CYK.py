...
def calcCYK(w):
    ...
    # For the induction.
    for rge in range(1, L):
        for bgn in range(L -rge):
            end = bgn + rge

            # Initialize X[bgn, end]
            for symbol in range(VarNum):
                X[bgn][end][symbol] = False

            # Handle X[bgn, end] = Union(X[bgn][pivot], X[pivot + 1][end]),
            #                      bgn <= pivot < end
            for pivot in range(bgn, end):
                for src in range(VarNum):
                    for dst_1 in range(VarNum):
                        for dst_2 in range(VarNum):
                            if X[bgn][pivot][dst_1] == False or \
                               X[pivot + 1][end][dst_2] == False:
                               continue
                            result = existProd(src, dst_1, dst_2)
                            if result == True:
                                X[bgn][end][src] = True
